class ThingServiceOfferingsController < ApplicationController
  include ActionController::Helpers
  helper ThingsHelper
  wrap_parameters :thing_service_offering, include: ["service_offering_id", "thing_id"]
  before_action :get_thing, only: [:index, :update, :destroy]
  before_action :get_service_offering, only: [:servic_offering_things]
  before_action :get_thing_service_offering, only: [:update, :destroy]
  before_action :authenticate_user!, only: [:create, :update, :destroy]
  after_action :verify_authorized
  #after_action :verify_policy_scoped, only: [:linkable_things]

  def index
    authorize @thing, :get_service_offerings?
    @thing_service_offerings = @thing.thing_images.with_public
  end
